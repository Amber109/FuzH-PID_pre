function [trainedNet, Fuzzy, info] = trainNetworkFuzzy(varargin)
...
try
    [layersOrGraph, opts, X, Y] = iParseInputArguments(varargin{1:end-1});
    [trainedNet, Fuzzy, info] = doTrainNetwork(layersOrGraph, opts, X, Y, varargin{end});
catch e
...
function [trainedNet, Fuzzy, info] = doTrainNetwork(layersOrGraph, opts, X, Y, Flag_Fuzzy)
...
if numel(networkInfo.InputSizes) == 1
    iValidateDataSizeForNetwork( trainingDispatcher, validationDispatcher, networkInfo, trainedNet );
else
...
% Do the training
[trainedNet Fuzzy] = trainer.trainFuzzy(trainedNet, trainingDispatcher, Flag_Fuzzy);
...
function iValidateDataSizeForNetwork(trainingDispatcher, validationDispatcher, networkInfo, internalNet)
% Assert that the training data is valid for the network architecture
trainingDataValidator = iTrainingDataValidator;
trainingDataValidator.validateDataSizeForNetwork( ...
    trainingDispatcher, networkInfo, internalNet);

% Assert that the validation data is valid for the network architecture
validationDataValidator = iValidationDataValidator;
validationDataValidator.validateDataSizeForNetwork( ...
...
trainingPlotBuilder = iCreateTrainingPlotBuilder(opts.Plots);
if ~isempty(trainingPlotBuilder)
    if isdeployed
    ...
    trainingPlotReporter = iCreateTrainingPlotReporter( trainingPlotBuilder, ...
        executionSettings, opts, networkInfo, trainingDispatcher, ...
        isValidationSpecified, validationReporter);
...
function trainingPlotBuilder = iCreateTrainingPlotBuilder(plotsOption)
if isa(plotsOption, 'nnet.internal.cnn.ui.TrainingPlotBuilder')
    trainingPlotBuilder = plotsOption;
elseif strcmp(plotsOption, 'training-progress')
    trainingPlotBuilder = nnet.internal.cnn.ui.TrainingPlotWithDialogBuilder();
else
    trainingPlotBuilder = [];
end
end

function trainingPlotReporter = iCreateTrainingPlotReporter(builder, executionSettings, opts, networkInfo, trainingDispatcher, isValidationSpecified, validationReporter)
builder.ExecutionEnvironment = executionSettings.executionEnvironment;
builder.UseParallel = executionSettings.useParallel;
builder.HasVariableNumItersPerEpoch = iHasVariableNumItersEachEpoch(opts, networkInfo, trainingDispatcher);
builder.NumObservations = trainingDispatcher.NumObservations;
builder.MaxEpochs = opts.MaxEpochs;
builder.MiniBatchSize = opts.MiniBatchSize;
builder.LearnRateSchedule = opts.LearnRateScheduleSettings.Method;
builder.InitialLearnRate = opts.InitialLearnRate;
builder.HasValidation = isValidationSpecified;
builder.ValidationFrequency = opts.ValidationFrequency;
builder.ValidationPatience = opts.ValidationPatience;
builder.LossType = iGetLossType(networkInfo);

% create the presenter
trainingPlotPresenter = builder.build();

% create the reporter
summaryFactory = nnet.internal.cnn.util.SummaryFactory();
trainingPlotReporter = nnet.internal.cnn.util.TrainingPlotReporter(trainingPlotPresenter, validationReporter, summaryFactory);
end

function type = iGetLossType(networkInfo)
if networkInfo.DoesClassification
    type = "classification";
else
    type = "regression";
...
function tf = iHasVariableNumItersEachEpoch(opts, networkInfo, trainingDispatcher)
isRNNWithNumericSeqLen = networkInfo.IsRNN && isnumeric(opts.SequenceLength);
